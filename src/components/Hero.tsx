/**
 * Hero.tsx
 * Landing hero section — premium editorial redesign.
 *
 * Animations (preserved + elevated):
 *   ① Logo        — fade-down entrance
 *   ② Headline    — word-by-word staggered fade-up (existing pattern, enhanced)
 *   ③ Subtitle    — word-by-word with blur dissolve
 *   ④ CTA buttons — spring entrance
 *   ⑤ Board col   — slide from right + rotation correction
 *   ⑥ Orbs        — CSS keyframe float + GSAP drift (preserved)
 *   ⑦ Tilt        — cursor-driven 3D (usePerspectiveTilt, preserved)
 *   ⑧ Board glow  — gold-tinted radial (upgraded from indigo)
 *
 * Visual upgrades:
 *   - Cormorant Garamond for h1 display
 *   - Editorial copy with more precise language
 *   - Board card: obsidian + gold hairline border, depth layers
 *   - Ambient light beams behind hero text
 *   - Section eyebrow label (DM Mono)
...
export default function Hero() {
  ...
  useGSAP(
    () => {
      ...
      // ── Custom SplitText Utility (preserved exactly) ───────────────────
      ...
      // ② Headline — cinematic stagger (slightly slower, more luxurious)
      tl.fromTo(
      ...
      // ⑤ Board column — cinematic entrance (preserved)
      tl.fromTo(
        boardColRef.current,
      ...
      // ── Background orbs drift (preserved) ─────────────────────────────
      ...
      // ── Play icon glow pulse (preserved) ─────────────────────────────
      ...
      // ── CTA text char animation on hover (preserved) ──────────────────
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ background: 'rgba(180, 147, 74, 0.03)' }}
          aria-hidden="true"
        />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 lg:items-center">
            ...
            <div className="lg:col-span-6 space-y-8 md:space-y-10 text-left">
              ...
              >
                <p
                  ref={subPara1Ref}
                  className="font-sans text-xl sm:text-2xl font-light text-ivory leading-relaxed"
                ...
                >
                  A complete chess platform — play, learn, compete, and grow.
                  Built to become the world's foremost destination for chess.
                </p>
              </div>
            ...
            */}
            ...
            >
              {/*
                tiltRef → usePerspectiveTilt owns:
